/**
 * Training & placement fees: 6% GST applied on the discounted (taxable) base.
 * DB columns `discounted_fee` / `placement_discounted_fee` store the base before GST.
 */

const TRAINING_PLACEMENT_GST_RATE = 0.06;

function roundMoney2(n) {
  return Math.round(Number(n) * 100) / 100;
}

function discountedTotalWithGst(baseAmount) {
  if (baseAmount == null || baseAmount === "") return null;
  const n = parseFloat(baseAmount);
  if (!Number.isFinite(n)) return null;
  return roundMoney2(n * (1 + TRAINING_PLACEMENT_GST_RATE));
}

function feeBalanceFromBaseDiscounted(baseDiscounted, feePaid) {
  const total = discountedTotalWithGst(baseDiscounted);
  if (total == null) return null;
  const paid = parseFloat(feePaid) || 0;
  return Math.max(0, roundMoney2(total - paid));
}

function placementBaseAmount(placement_discounted_fee, placement_fee) {
  if (
    placement_discounted_fee != null &&
    placement_discounted_fee !== "" &&
    !Number.isNaN(parseFloat(placement_discounted_fee))
  ) {
    const d = parseFloat(placement_discounted_fee);
    if (Number.isFinite(d)) return d;
  }
  if (
    placement_fee != null &&
    placement_fee !== "" &&
    !Number.isNaN(parseFloat(placement_fee))
  ) {
    const f = parseFloat(placement_fee);
    if (Number.isFinite(f)) return f;
  }
  return null;
}

function placementBalanceAfterGst(
  placement_discounted_fee,
  placement_fee,
  placement_paid
) {
  const base = placementBaseAmount(placement_discounted_fee, placement_fee);
  if (base == null) return null;
  const total = discountedTotalWithGst(base);
  if (total == null) return null;
  const paid = parseFloat(placement_paid) || 0;
  return Math.max(0, roundMoney2(total - paid));
}

module.exports = {
  TRAINING_PLACEMENT_GST_RATE,
  roundMoney2,
  discountedTotalWithGst,
  feeBalanceFromBaseDiscounted,
  placementBaseAmount,
  placementBalanceAfterGst,
};
