...
import {
  ...
  STATUS_TABS,
  TAB_ACTIVE_CLASS,
} from "./constants/studentEnrollmentsConstants";
import { useStudentEnrollmentsController } from "./hooks/useStudentEnrollmentsController";

export default function StudentEnrollmentsMain() {
  const {
    loading,
    filterStatus,
    setFilterStatus,
    ...
    displayList,
    countFor,
  } = useStudentEnrollmentsController();
    ...
    <div className="p-6 bg-[#f0f1f5] min-h-screen">
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        ...
        <button
          ...
          className="ml-auto flex items-center gap-2 px-4 py-2 bg-white border border-gray-200 rounded-xl text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors shadow-sm"
        >
                ...
                <tr>
                  <td colSpan="5" className="px-6 py-12 text-center text-gray-500">
                    <div className="flex flex-col items-center gap-2">
                      <MdRefresh className="text-4xl animate-spin text-blue-500" />
                      Loading enrollments...
                    </div>
                  </td>
                </tr>
                  ...
                  <tr key={e.enrollment_id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4">
                      ...
                      <div className="text-xs text-gray-400">
                        {e.country_code} {e.mobile_number}
                      </div>
                    </td>
                    <td className="px-6 py-4">
                        ...
                        {e.status === "submitted" && (
                          <>
                            <button
                              ...
                              className="flex items-center gap-1 px-2.5 py-1.5 text-xs font-bold text-white bg-cyan-600 hover:bg-cyan-700 rounded-lg transition-colors"
                            >
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
                ...
                <div className="w-14 h-14 rounded-full overflow-hidden bg-gray-100 flex-shrink-0 border-2 border-gray-200 flex items-center justify-center">
                  {selectedEnrollment.photo_url ? (
                    ...
                    <span className="text-gray-400 text-xl font-bold">
                      {selectedEnrollment.name?.charAt(0)?.toUpperCase() || "S"}
                    </span>
                  )}
                </div>
                <div>
                  <h2 className="text-lg font-bold text-gray-900">{selectedEnrollment.name}</h2>
                  <p className="text-sm text-gray-500">{selectedEnrollment.email || "No email"}</p>
                  <div className="mt-1">
            ...
            <div className="px-6 pb-5 max-h-[60vh] overflow-y-auto">
              <div className="grid grid-cols-2 gap-x-8 gap-y-4 py-4 border-t border-gray-100">
                {[
                  ["Mobile", `${selectedEnrollment.country_code} ${selectedEnrollment.mobile_number}`],
                  ["Course Applied", selectedEnrollment.course_name || selectedEnrollment.course_id],
                  ["Location", selectedEnrollment.location],
                  ["College / Company", selectedEnrollment.college],
                  ["Languages", selectedEnrollment.language],
                  ["School/College ID", selectedEnrollment.school_college_id],
                  [
                    "Applied Date",
              ...
              <div className="py-4 border-t border-gray-100">
                ...
                <p className="text-sm text-gray-600 leading-relaxed bg-gray-50 rounded-lg px-4 py-3 border border-gray-100">
                  {selectedEnrollment.additional_info || "No additional information provided."}
                </p>
              </div>
            ...
            <div className="px-6 py-4 border-t border-gray-100 flex items-center justify-end gap-3">
              {!showRejectionInput && selectedEnrollment.status === "submitted" && (
                <>
                  ...
                  >
                    {actionLoading ? <MdRefresh className="animate-spin" /> : <MdThumbUp />}
                    Accept Application
                  </button>
                </>
              )}
              {!showRejectionInput && selectedEnrollment.status === "accepted" && (
                <>
                  ...
                  >
                    {actionLoading ? <MdRefresh className="animate-spin" /> : <MdCheckCircle />}
                    Approve Enrollment
                  </button>
                </>
              )}
              {showRejectionInput && (
                <>
                  ...
                  >
                    {actionLoading ? <MdRefresh className="animate-spin" /> : null}
                    Confirm Rejection
                  </button>
                </>
              )}
